...
const Login = () => {
    ...
    const loginuser = async (e) => {
        ...
        try {
            ...
            const info = [data.role, data.user_id , data?.token];

            if (response.ok) {
                setsucces(`Connecté en tant que ${data.role}`);
                if (data.token) {
                    setToken(data.token);
                    console.log(data.token); // ← Utilise directement la donnée reçue
                    localStorage.setItem('token', data.token); // Stocke directement
                }
